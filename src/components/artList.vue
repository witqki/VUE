       ...
       <!-- <img :src="songList[0].album.picUrl" :alt="songList[0].album.name" class="avatar">-->
        <div class="data__cont">
          <div class="data__name">
            <h1>{{songList[0].name}}</h1>
            <p>歌手描述：{{songList[0].singer}}</p>
            <p>歌曲年份：暂无</p>
            <p>描述：暂无</p>
          </div>
        </div>
            ...
            <li v-for="(item, index) in songList">
              ...
              <div class="songName">
                ...
                <div class="mod_list_menu noShow" @mouseover="iconShow">
                   ...
                   <i class="song_add" @click="addmusic(index)"></i>
                    <!-- <audio v-bi>
                     下载
                    </audio> -->
                    <i class="song_download" @click="downloadmusic(index)"></i>
                    
              <!-- <audio v-bi>
                      分享
                    </audio> -->
                 <i class="song_share" @click="sharemusic(index)"></i>

                 <!-- <audio v-bi>
                      分享
                    </audio> -->
               <i class="song_comment" @click="commentmusic(index)"></i> 
                </div>
                
              
              </div>
              <p class="songAlbum">{{item.album}}</p>
              <p class="songTime">{{transformTime(item.time)}}</p>
            </li>
          </ul>
        </div>
      </div>
...
import {mapState} from 'vuex'
  export default {
    ...
    methods: {
      ...
      playSong(index){
         this.$store.commit('save_index', index)
        console.log("success]"+this.$store.state.index+index)
...
<style lang="scss" scoped>
  ...
  .songlist__list {
    ...
    .mod_list_menu {
      ...
      top: 0;
      right: 10%;
      .song_play,.song_download, .song_share,.song_add,.song_comment{
        width: 36px;
        height: 36px;
        display: block;
        float: left;
